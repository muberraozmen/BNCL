...
if __name__ == '__main__':
    ...
    for annotation_ratio in [1, 10, 20, 30, 40, 50]:
        ...
        for seed in range(1, 11,1):
            ...
            results_dir = args.results_dir + '/supervision level ' + str(args.supervision) + '/seed ' + str(seed) + time.strftime(' on %m.%d.%Y at %H.%M.%S/')
            ...
            data = TransformedData(**vars(args), annotation_ratio=annotation_ratio)
            ...
            for i, (key, value) in enumerate(metrics_best.items()):
                #logger.info('- {:} = {:}'.format(key, np.round_(value, decimals=4)))
                if key not in results[annotation_ratio].keys():
                    results[annotation_ratio][key] = []
                results[annotation_ratio][key].append(value)
